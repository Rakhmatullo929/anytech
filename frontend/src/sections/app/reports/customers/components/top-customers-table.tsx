...
export default function TopCustomersTable({ dateFrom, dateTo }: Props) {
  const { tx } = useLocales();
  ...
  const { data, isPending, isFetching } = useTopCustomersQuery(params);
  ...
  const tableHead = useMemo(
    () => [
      ...
      { id: 'totalSpent', label: tx('reports.tables.totalSpent') },
    ],
    [tx]
  );
